...
|---------|------|-------------|
| 4.5.1 | 2026-04-03 | OmniRoute integration: LLMClient auto-detects OMNIROUTE_BASE_URL and routes requests through custom proxy instead of OpenRouter. Fixes provider lock-in, enables free/self-hosted model routing. llm.py, config.py, pricing.py updated with provider detection and correct header handling. |
